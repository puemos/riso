defmodule Riso.CampaignsTest do
  ...
  describe "campaigns_members" do
    ...
    test "list_campaigns_members/0 returns all campaigns_members" do
      campaign_member = campaign_member_fixture()
      assert Campaigns.list_campaigns_members() == [campaign_member]
    end

    test "get_campaign_member!/1 returns the campaign_member with given id" do
      campaign_member = campaign_member_fixture()
      assert Campaigns.get_campaign_member!(campaign_member.id) == campaign_member
    end

    test "create_campaign_member/1 with valid data creates a campaign_member" do
      assert {:ok, %CampaignMember{} = campaign_member} = Campaigns.create_campaign_member(@valid_attrs)
